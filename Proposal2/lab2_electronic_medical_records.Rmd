...
note_id :  A unique identifier assigned to each individual medical note in the dataset.

description : A brief summary of each patient containing an identifier

medical_specialty :  The field of medicine that the note pertains to, such as cardiology, orthopedics, or dermatology.

sample_name : The name or identifier of the sample patient or case associated with the medical note.

transcription : Fulltext of the medical note, including clinical observations, diagnoses, and treatment plans.

keywords : Specific terms or phrases extracted from the transcription that highlight important aspects or topics covered in the medical note
...
No I do not think a general purpose lemmatizer will work well for medical data.

General purpose lemmatizers lack specific context cues and medical terminologies which in turn leads to innacuracies in processing medical texts.
...
Based on what these specialties involve this is about what is expected.

This plot shows that lemma frequencies between Surgery and Orthopedic are similar. In contrast, Radiology and Orthopedic have less similarity, with some more scattered points. This makes sense because Surgery and Orthopedic share common terms, while Radiology uses different language related to imaging.
...
```{r}
ggplot2::ggplot(lemma.comp, ggplot2::aes(x=proportion, 
                                         y=`Surgery`,
...
Out of all the terms the one that stands out the most is department anesthesia.

This stands out the most to me because unlike the other terms that are specifically related to orthopedic techniques while department anesthesia is a general term applicable to all surgical departments and procedures.
...
```{r}
analysis.data %>% dplyr::select(keywords, description) %>% dplyr::filter(stringr::str_detect(keywords, 'surgery')) %>% dplyr::slice(1)
```
...
```{r}
top_terms_3_topic <- top.terms %>% 
  filter(topic %in% c(1, 2, 3)) %>%
  pull(term)

top_terms_6_topic <- top.terms6 %>%
  filter(term %in% top_terms_3_topic)

top_terms_6_topic

```

The top two terms appear, "prepped drapes and preoperative diagnoss" appear but the last of the top three is missing.
