...
According to the dictionary pneumonia is a lung inflammation caused by bacterial or viral infection, in which the air sacs fill with pus and may become solid. Inflammation may affect both lungs (double pneumonia) or only one (single pneumonia).

A benefit of being able to directly be able to identify it from xrays is fast and effective treatment, if after examining an xray and glaringly being able to tell the cause of an illness (in this case pneumonia) then it can quickly be treated before getting worse.
...
These filenames indicate that the x-ray images are classified as either normal or pneumonia with some cases further identified by type of infection, the labelling allows us to seperate healthy from pneumonia affected x-rays.


Predicting pneumonia might be made more challenging because these images firstly give an idea but in the case of new data a model might be compromised due to the ability to not be able to identify issues.
...
The images could be made clearer and more augmented (the quality)

The images could be resized to make sure all the images are of the same size and consistent throughout. (the size)

The same tone across all boards with consistent colour grading. (the image colour)
...
Perhaps we could use some resampling of the data being used (resampling techniques)

Assign higher weights to the minority groups in the dataset.

Various transformative techniques could be used to images to create larger and more diverse datasets.

Different algorithms to analyze and handle imbalanced datasets.
...
Some data augmentation methods could distort crucial data which could lead to loss of important data.

Some unintended biases may be introduced to the dataset which affects performance.

Data augmentation is very time consuming as making sure all things involved are good for analysis is tedious.

Augmentation could lead to overfitting, instead of learning general features.
...
ImageNet is a largew scale image database created for use in visual object recognition software research, containing over 13 million labeled images spanning a variety of categories (roughly 21,000). This dataset helps in training deep learning models by providing diverse and rich visual information which helps in learning grand and generalizable features.
...
ImageNet can help improve results in this assignment because by using the pre-trained models it can enhance results in the medical imaging analysis and the diverse dataset allows the model to learn general features which will be very useful for the different image types which in turn speeds up analysis and creates better results.

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?

There are about 4.2 million parameters which means it is much smaller than many other deep neural network architectures.

Due to the smaller nature of this network it allows for faster training and analysis and compared to the better performing networks available in torch it has fewer parameters which means lower performance but faster execution.
